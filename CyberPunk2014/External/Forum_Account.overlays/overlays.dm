...
atom
	...
	proc
		overlay(ntag, icon, icon_state = null, layer = null)

			if(ntag && overlayObjects[ntag])
				return overlayObjects[ntag]
			...
			return new /Overlay(src, icon, icon_state, layer, ntag)

		getOverlay(ntag)
			if(ntag in overlayObjects) return overlayObjects[ntag]
...
Overlay
	...
	New(mob/o, icon, icon_state = null, layer = null, ntag)
		src.tag = ntag

		o.overlayObjects[tag] = src
	...
	proc
		...
		Alpha()
			...
			if(image_obj)
				image_obj.alpha = newAlpha
			else
				if(visible) owner.overlays -= overlay_obj
				overlay_obj.alpha = newAlpha
				if(visible) owner.overlays += overlay_obj
		...
		Flick(is, duration)
			...
			if(image)
				...
				image_obj.icon_state = is
				isFlicking = 1
				spawn(duration)
					isFlicking = 0
					image_obj.icon_state = overlay_obj.icon_state
			else
